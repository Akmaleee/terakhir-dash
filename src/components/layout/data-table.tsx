...
export function DataTable({
  ...
  const handleCustomAction = async (action: string, row: any) => {
    // 💡 Kirim aksi ke page.tsx jika ada custom action handler di parent
    if (onCustomAction) {
      onCustomAction(action, row);
    }

    // 👉 NDA/MOU/MSA khusus upload Sirkulir dan Signed
    ...
    // Aksi-aksi ini ditangani di page.tsx, jadi jangan panggil API progress di sini
    if (
      action === "Print" ||
      action === "Check" ||
    ...
    // 🔥 PERBAIKAN UTAMA DI SINI (Send, Approve, Sign)
    try {
      // 1. Panggil API Progress
      ...
      // 2. Tentukan URL endpoint yang benar berdasarkan 'type'
      let endpointType = type.toLowerCase();
      
      // Mapping eksplisit jika diperlukan (sesuai nama file di /app/api/...)
      if (endpointType === "mou") endpointType = "mou";
      else if (endpointType === "msa") endpointType = "msa";
      else if (endpointType === "nda") endpointType = "nda";
      else if (endpointType === "mom") endpointType = "mom";
      else if (endpointType === "jik") endpointType = "jik";
      
      const refetchUrl = `/api/${endpointType}`;
      console.log(`🔄 Refreshing data from: ${refetchUrl}`);

      // 3. Re-fetch data terbaru dari server (No Cache)
      ...
      // 4. Update State Rows dengan data baru
