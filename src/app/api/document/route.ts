  ...
  try {
    ...
    let progress = await prisma.progress.create({
        data: {
            ...
            step_id: STEP_MAP[step_name as keyof typeof STEP_MAP] ?? null,
            status_id: 1, // Default status (Draft/Processing)
        },
    });
    ...
    const document = await prisma.document.create({
      ...
      include: {
        progress: {
          include: {
            company: true,
            step: true,   // 🔥 WAJIB ADA: Agar kolom "Jenis Dokumen" muncul
            status: true, // 🔥 WAJIB ADA: Agar status tracker muncul
          },
        },
      },
    });
